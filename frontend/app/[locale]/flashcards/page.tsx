...
// Define interfaces para mejorar el tipado
...
// Component that specifically handles navigation hooks
function NavigationHandler({ 
  children 
}: { 
  children: (props: NavigationProps) => ReactNode
}) {
  // Agrupamos todos los hooks de navegación en un componente separado
  // que estará envuelto en Suspense
  const pathname = usePathname()
  const searchParams = useSearchParams()
  const router = useRouter()
  
  return <>{children({ pathname, searchParams, router })}</>
}
...
function FlashcardsContent() {
  ...
  // Estado local para manejar navegación sin usar los hooks directamente
  const [localPathname, setLocalPathname] = useState<string>('')
  
  const setupComponent = (navigationProps: NavigationProps) => {
    const { pathname, router } = navigationProps;
    
    // Guardar pathname localmente para usar más tarde
  ...
  return (
    <Suspense fallback={<div>Loading navigation context...</div>}>
      <NavigationHandler>
        {(navigationProps) => setupComponent(navigationProps)}
      </NavigationHandler>
    </Suspense>
  );
}
